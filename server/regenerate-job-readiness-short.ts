...
async function generateShorterLessonContent(
  moduleTitle: string,
  lessonTitle: string,
  lessonNumber: number
...
2. Practical with real examples
3. Comprehensive coverage of the topic
4. Suitable for someone preparing for employment
...
export async function regenerateJobReadinessShortContent() {
  console.log('🔄 Starting Job Readiness content regeneration with shorter lessons...');
  ...
  for (const mod of courseModules) {
    ...
    for (let i = 0; i < moduleLessons.length; i++) {
      ...
      try {
        const generated = await generateShorterLessonContent(
          mod.title,
          lesson.title,
          i + 1
        );
        ...
        console.log(`    ✅ Updated with shorter content`);
        totalUpdated++;
  ...
  console.log(`\n🎉 Regeneration complete! Updated ${totalUpdated} lessons with shorter content.`);
}

regenerateJobReadinessShortContent()
  .then(() => process.exit(0))
  .catch((err) => {
