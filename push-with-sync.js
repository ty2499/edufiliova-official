...
async function main() {
  try {
    ...
    console.log("🚀 Syncing and pushing source code\n");
    ...
    console.log(`✅ Latest SHA on GitHub: ${baseSha}\n`);
    ...
    for (let i = 0; i < allFiles.length; i += batchSize) {
      ...
      try {
        // Create tree with LATEST baseSha
        ...
        baseSha = newCommit.sha; // Update baseSha for next batch
        successCount += batch.length;
        console.log(`   ✅ Batch ${batchNum} complete (${batch.length} files)`);
        batchNum++;
      } catch (error) {
        console.error(`   ⚠️  Batch ${batchNum} failed:`, error.message);
        batchNum++;
      }
    }

    console.log(`\n✅ Pushed ${successCount}/${allFiles.length} files to GitHub`);
