...
async function pushFile(path: string, localPath: string, message: string) {
  ...
  console.log('✅ Pushed:', path, result.data.commit.html_url);
}

pushFile('cordova-app/config.xml', '/home/runner/workspace/cordova-app/config.xml', 
  'Update config.xml for cordova-android 12 compatibility').catch(console.error);
